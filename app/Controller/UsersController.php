...
class UsersController extends AppController {
    ...
    public function add() {
        ...
        $groups = $this->User->UserGroup->find('list');
...
//                $user['User']['salt'] = $salt;
//                $user['User']['password'] = $this->UserAuth->makePassword('123456', $salt);
//                if($this->User->save($user, false)){
//					echo $u['User']['username'].' ok<br>';
//				}else{
//					echo $u['User']['username'].' not oke<br>';
//				}
//
//		}
//
...
//
    ...
    public function lockscreen() {
        $this->layout = 'login';
        $user_id = $id = $this->UserAuth->getUserId();
        $user_foto = $id = $this->UserAuth->getUserFoto();
        $username = $id = $this->UserAuth->getUserName();

        if ($this->request->is('post')) {
            if ($this->request->data['User']['username'] == '' || $this->request->data['User']['password'] == '') {
                $this->Session->setFlash(__('Email/Username and Password mus not blank'));
                return;
            } else {
...
// check for verified account
                ...
                if ($user['User']['password'] === $hashed) {
                    ...
                    $this->Session->setFlash(__('Incorrect Email/Username or Password'), 'red');
                    return;
                }
            }
        }
        $this->set(compact('user_foto', 'user_id', 'username'));
...
// check for verified account
    ...
    public function update_profile() {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            $localDir = '/img/foto_profile/';
            $upload_dir = dirname($_SERVER['SCRIPT_FILENAME']) . $localDir;
            $error = 0;
            $data = $this->request->data;

            $desirable_sizes = array(150, 100, 60, 30);

            $now = time();
            $sizes = getimagesize($data['User']['foto']['tmp_name']);
            $extensions['jpeg'] = '.jpg';
            $extensions['.jpg'] = '.jpg';
            $extensions['.gif'] = '.gif';
            $extensions['.png'] = '.png';
            $dat['User']['prev_foto'] = $data['User']['prev_foto'];
            $dat['foto']['name'] = $data['User']['foto']['name'];
            if ($dat['foto']['name'] != ''):
                $dat['foto']['size'] = $data['User']['foto']['size'];
                $dat['foto']['width'] = $sizes[0];
                $dat['foto']['height'] = $sizes[1];
                $dat['foto']['type'] = substr(strrchr($data['User']['foto']['name'], '.'), 1);
                $dat['foto']['ext'] = substr($data['User']['foto']['name'], -4);
                $dat['foto']['foto'] = $now . $dat['foto']['ext'];
                $dat['foto']['tmp_name'] = $data['User']['foto']['tmp_name'];
                $dat['foto']['upload_dir'] = $upload_dir;
                $dat['foto']['full_url'] = $upload_dir . $now . $dat['foto']['ext'];
                foreach ($desirable_sizes as $s) {
                    $dat['foto']['resized_' . $s] = $upload_dir . $now . "_" . $s . $dat['foto']['ext'];
                }
                $dat['User']['foto'] = $dat['foto']['foto'];
//                echo '<pre>';
//                print_r($data);
//                echo '</pre>';
//                exit;
                if (!isset($extensions[$dat['foto']['ext']])) {
                    $err = "file yang diupload adalah type .jpg, .gif, atau .png!";
                    $error = 1;
                    $this->Session->setFlash(__($err));
                } elseif ($dat['foto']['size'] > 300 * 1024) {
                    $err = "ukuran file terlalu besar. maksimal adalah 300Kb!";
                    $error = 1;
                    $this->Session->setFlash(__($err));
                } elseif ($dat['foto']['width'] != 750 && $dat['foto']['width'] != 300) {
                    $err = 'ukuran foto adalah lebar 750px dan tinggi 300px. silakan ubah ukurannya dengan menggunakan program PAINT';
                    $error = 1;
                    $this->Session->setFlash(__($err));
                }
                move_uploaded_file($dat['foto']['tmp_name'], $dat['foto']['full_url']);
                foreach ($desirable_sizes as $s) {
                    unlink($upload_dir . str_replace('.', '_', $s . '.', $dat['User']['prev_foto']));
                }
                unlink($upload_dir . $dat['User']['prev_foto']);
                foreach ($desirable_sizes as $s) {
                    $this->Util->Resize($dat['foto']['full_url'], $dat['foto']['resized_' . $s], $dat['foto']['ext'], $s);
                }
            endif;
            ...
            if ($this->request->isPost()) {
                ...
                if ($this->User->RegisterValidate()) {
                    if (!isset($this->request->data['User'][
                                    'user_group_id'])) {
                        $this->request->data['User'] ['user_group_id'] = 5;
                    } elseif (!$this->UserGroup->isAllowedForRegistration($this->request->data['User']['user_group_id'])) {
                        $this->Session->setFlash(__('Please select correct register as'));
                        return;
                    }
     ...
     * @return void
     */
    public function change_user_password($userId = null) {
        if (!empty($userId)) {
            ...
            if (!empty($user)) {
                if (!$user['User']['email_verified']) {
                    ...
                    if ($activateKey == $theKey
                        ...
                        $this->
                        Util->setFlash(__('Thank you, your account is activated now'));
                    }
                } else {
    ...
    public function change_password($id = null) {
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if ($this->request->data['User']['password'] == '' || $this->request->data['User']['password2'] == '') {
                $this->Session->setFlash(__('Password dan konfirmasi passwword harus diisi'));
            } elseif ($this->request->data['User']['password'] != $this->request->data['User']['password2']) {
                $this->Session->setFlash(__('Konfirmasi password tidak cocok'));
            } else {
//echo '<pre>';
                if ($id == '') {
                ...
                if ($result) {
                    $this->Session->setFlash(__('Ubah Password berhasil'));
                    $this->redirect(array('action' => 'index'));
                } else {
                    $this->Session->setFlash(__('The user could not be saved. Please, try again.'));
                }
    ...
    public function reset_password($id = null) {
        ...
        $duser = $this->User->read(null, $id);
        if ($duser['User']['group_id'] == 1) {
            $action = 'index';
        } else {
            $action = 'member';
        }
