...
class UsersController extends UserMgmtAppController {
     ...
     * @var array
     */
    public $uses = array('Usermgmt.User', 'Usermgmt.UserGroup', 'Usermgmt.LoginToken', 'Usermgmt.SocialProfile');
    ...
    public function myprofile() {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if ($dat['avatar']['name'] != ''):
                ...
                $dat['avatar']['resized_100'] = $upload_dir . $now . "_100" . $dat['avatar']['ext'];
                $dat['avatar']['resized_50'] = $upload_dir . $now . "_50" . $dat['avatar']['ext'];
                $dat['avatar']['resized_30'] = $upload_dir . $now . "_30" . $dat['avatar']['ext'];
                $dat['User']['avatar'] = $dat['avatar']['avatar'];
                if (!isset($extensions[$dat['avatar']['ext']])) {
                    $err = "file yang diupload adalah type .jpg, .gif, atau .png!";
                    $error = 1;
                    $this->Util->setFlash(__($err));
                ...
                $this->Util->Resize($dat['avatar']['full_url'], $dat['avatar']['resized_100'], $dat['avatar']['ext'], 100);
                $this->Util->Resize($dat['avatar']['full_url'], $dat['avatar']['resized_50'], $dat['avatar']['ext'], 50);
                $this->Util->Resize($dat['avatar']['full_url'], $dat['avatar']['resized_30'], $dat['avatar']['ext'], 30);
                $dat['User']['id'] = $userId;
                @unlink($localDir . $old_avatar);
                @unlink($localDir . str_replace('.', '_100.', $old_avatar));
                @unlink($localDir . str_replace('.', '_500.', $old_avatar));
            endif;

            if ($result = $this->User->save($dat)) {
                $this->Session->write('UserAuth.User.avatar', $dat['User']['avatar']);
                $this->Util->setFlash(__('The user has been saved'));
                return $this->redirect(array('action' => 'myprofile'));
            } else {
                $this->Util->setFlash(__('The user could not be saved. Please, try again.'));
            }
        } else {
            $user = $this->User->read(null, $userId);
            $this->set('user', $user);
...
// check for verified account
                ...
                if ($user['User']['password'] === $hashed) {
                    ...
                    $this->loadModel("Usermgmt.LoginLog");
                    $d['LoginLog']['user_id'] = $user['User']['id'];
                    $d['LoginLog']['ip'] = $this->request->clientIp();
                    $this->LoginLog->save($d);
        ...
        if (SITE_REGISTRATION) {
            ...
            if ($this->request->isPost()) {
                ...
                if ($this->User->RegisterValidate()) {
                    ...
                    $this->request->data['User']['username'] = $this->request->data['User']['first_name'] . '_' . $this->request->data['User']['first_name'];
...
// check for inactive account
    ...
    public function social_login($provider) {
        if ($this->Hybridauth->connect($provider)) {
            ...
            var_dump($this->Hybridauth->error);
            die;
            // error
            $this->Util->setFlash($this->Hybridauth->error);
            $this->redirect('/');
        }
    }
    ...
    private function _successfulHybridauth($provider, $incomingProfile) {
        ...
        if ($existingProfile) {
            ...
            if ($this->UserAuth->isLogged()) {
                ...
                $incomingProfile['SocialProfile']['user_id'] = $this->UserAuth->getUserId();
                $this->SocialProfile->save($incomingProfile);

                $this->Util->setFlash('Your ' . $incomingProfile['SocialProfile']['social_network_name'] . ' account is now linked to your account.');
                $this->redirect($this->UserAuth->redirectUrl());
            } else {
    ...
    private function _doSocialLogin($user, $returning = false) {
        if ($this->UserAuth->login($user)) {
            if ($returning) {
                $this->Util->setFlash(__('Welcome back, ' . $this->UserAuth->user('first_name')));
            } else {
                $this->Util->setFlash(__('Welcome to our community, ' . $this->UserAuth->user('first_name')));
            }
            $this->redirect(LOGIN_REDIRECT_URL);
        } else {
            $this->Util->setFlash(__('Unknown Error could not verify the user: ' . $this->UserAuth->user('username')));
        }
    }
